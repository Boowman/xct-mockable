...
import Foundation
import XCTSharkBridge

public class InvocationsRecorder: NSObject {
    private static let sharedQueue = DispatchQueue(label: "com.shark.mockable.invocationsRecorder")
    
    enum Constants {
        static let recorderKey = DispatchSpecificKey<InvocationsRecorder>()
        static let exception = NSException(
            name: NSExceptionName(rawValue: "com.shark.mockable.invocationsRecorder.result"),
            reason: nil,
            userInfo: nil)
    }
